...
std::string Explosion::exposion_anim_files[] = {
	...
	"regularExplosion06.png","regularExplosion06.png",
	"regularExplosion07.png","regularExplosion08.png"
};

Explosion::Explosion(sf::Vector2f pos) {
	texture.loadFromFile(IMAGES_FOLDER + exposion_anim_files[0]);
...
void Explosion::update() {
	...
	if (now.asMilliseconds() > frame_delay) {
		...
		if (frame < 9) {
			clock.restart();
			texture.loadFromFile(IMAGES_FOLDER + exposion_anim_files[frame]);
			sprite.setTexture(texture);
			sprite.setPosition(position);
		}
		else {
